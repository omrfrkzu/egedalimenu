    ...
    <div className="menu-view">
      ...
      {showCreateButton && (
        <div className="menu-create-table">
          ...
          {isCustomerMode && selectedTable && orders[selectedTable] && orders[selectedTable].length > 0 && (
            <div className="menu-create-table customer-complete-order">
              <h3 className="mobile-cart-title">Siparişlerim</h3>
                    ...
                    <div className="mobile-cart-item-image">
                      {item.image && item.image.trim() !== '' ? (
                        <>
                          ...
                          />
                          <div className="mobile-cart-item-image-fallback" style={{ display: 'none' }}>
                            🍽️
                          </div>
                        </>
                      ) : (
                        <div className="mobile-cart-item-image-fallback">🍽️</div>
                      )}
                      <div className="mobile-cart-item-quantity-badge">
                        {item.quantity}
                      </div>
                    </div>
                    <div className="mobile-cart-item-info">
                      <h4 className="mobile-cart-item-name">{item.name}</h4>
                      <p className="mobile-cart-item-price">{(item.price * item.quantity).toFixed(2)} ₺</p>
                      <div className="mobile-cart-item-controls">
                        <button
                          className="mobile-cart-quantity-btn"
                          onClick={() => onUpdateQuantity && onUpdateQuantity(selectedTable, item.id, -1)}
                        >
                          <Minus size={16} />
                        </button>
                        <span className="mobile-cart-quantity">{item.quantity}</span>
                        <button
                          className="mobile-cart-quantity-btn"
                          onClick={() => onUpdateQuantity && onUpdateQuantity(selectedTable, item.id, 1)}
                        >
              ...
              <div className="customer-order-summary">
                <div className="order-summary-item">
                  ...
                  <strong>{orders[selectedTable].reduce((sum, item) => sum + item.quantity, 0)} adet</strong>
                </div>
                <div className="order-summary-item total-price">
                  <span>Toplam Tutar:</span>
                  <strong>{orders[selectedTable].reduce((sum, item) => sum + item.price * item.quantity, 0).toFixed(2)} ₺</strong>
                </div>
              </div>
              
              <div className="customer-order-note">
                <label htmlFor="order-note-mobile">Not (Opsiyonel)</label>
                <textarea
        ...
        {isCustomerMode && selectedTable && orders[selectedTable] && orders[selectedTable].length > 0 && (
          <div className="customer-order-sidebar">
            <div className="customer-order-sidebar-content">
                    ...
                    <div className="sidebar-item-image">
                      {item.image && item.image.trim() !== '' ? (
                        <>
                          ...
                          />
                          <div className="sidebar-item-image-fallback" style={{ display: 'none' }}>
                            🍽️
                          </div>
                        </>
                      ) : (
                        <div className="sidebar-item-image-fallback">🍽️</div>
                      )}
                      <div className="sidebar-item-quantity-badge">
                        {item.quantity}
                      </div>
                    </div>
                    <div className="sidebar-item-info">
                      <h4 className="sidebar-item-name">{item.name}</h4>
                      <p className="sidebar-item-price">{(item.price * item.quantity).toFixed(2)} ₺</p>
                    </div>
                    <div className="sidebar-item-controls">
                      <button
                        className="sidebar-quantity-btn"
                        onClick={() => onUpdateQuantity && onUpdateQuantity(selectedTable, item.id, -1)}
                      >
                        <Minus size={16} />
                      </button>
              ...
              <div className="customer-order-note">
                <label htmlFor="order-note-sidebar">Not (Opsiyonel)</label>
                <textarea
                  id="order-note-sidebar"
                  className="order-note-input"
                  placeholder="Siparişinizle ilgili özel bir not ekleyebilirsiniz..."
                  value={orderNote}
                  onChange={(e) => setOrderNote(e.target.value)}
